...
export const test = (req, res) => {
  res.json({
    message: 'Rota API funcionando!',
  });
};

export const updateUser = async (req, res, next) => {
  if (req.user.id !== req.params.id)
    return next(errorHandler(401, 'Você só pode atualizar a sua conta!'));
  try {
...
export const getUserListings = async (req, res, next) => {
  if (req.user.id === req.params.id) {
    ...
    return next(errorHandler(401, 'Você só ver os seus próprios anúncios!'));
  }
};
